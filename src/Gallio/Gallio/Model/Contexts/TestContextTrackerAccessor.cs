...
{
    /// <summary>
    /// Static service locator for <see cref="StubTestContextTracker" />.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Handles the case where the runtime is not initialized by returning a
    /// <see cref="ITestContextTracker" />.
    /// </para>
    /// </remarks>
            ...
            get
            {
                if (cachedContextTracker == null)
                {
                    if (RuntimeAccessor.IsInitialized)
                        cachedContextTracker = RuntimeAccessor.ServiceLocator.Resolve<ITestContextTracker>();
                    else
